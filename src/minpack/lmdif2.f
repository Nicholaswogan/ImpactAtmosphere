...
c     subroutine lmdif2
c
c     the purpose of lmdif2 is to minimize the sum of the squares of
...
c
c       subroutine lmdif2(fcn2,m,n,x,fvec,tol,info,iwa,wa,lwa)
c
c     where
c
c       fcn2 is the name of the user-supplied subroutine which
c         calculates the functions. fcn2 must be declared
c         in an external statement in the user calling
c         program, and should be written as follows.
c
c         subroutine fcn2(m,n,x,fvec,iflag)
c         integer m,n,iflag
...
c         end
c
c         the value of iflag should not be changed by fcn2 unless
c         the user wants to terminate execution of lmdif2.
...
c         value of iflag. see description of fcn2. otherwise,
...
c
c         info = 5  number of calls to fcn2 has reached or
c                   exceeded 200*(n+1).
c
...
c     **********
      ...
      double precision epsfcn2,factor,ftol,gtol,xtol,zero
...
C      maxfev = 200*(n + 1)
      ...
      nprint = 0
      mp5n = m + 5*n
      call lmdif(fcn2,m,n,x,fvec,ftol,xtol,gtol,maxfev,epsfcn2,wa(1),
   ...
   10 continue
      return
c
c     last card of subroutine lmdif2.
c
      end
