module climate
  ...
  real(real_kind), parameter :: poly(4) = [295.74510905d0, 76.02965729d0, &
...
contains
  ...
  subroutine pahlevan_H2_clima(P_surf, mubar, grav, P_top, nz, &
    ...
    real(real_kind), intent(in) :: P_surf ! bars
    real(real_kind), intent(in) :: mubar ! g/mol
    real(real_kind), intent(in) :: grav ! cm/s2
    real(real_kind), intent(in) :: P_top ! bars
    integer, intent(in) :: nz 
    
    real(real_kind), intent(out) :: zout(nz) ! cm
    real(real_kind), intent(out) :: Pout(nz) ! bar
    real(real_kind), intent(out) :: Tout(nz) ! K
    real(real_kind), intent(out) :: ztrop ! cm
