...
# @nb.njit
def obj_all1(x, nz, z, mubar_dry, M_dry, grav):
    P0 = 10.0**x[0]
    PH2O = 10.0**x[1:nz+1]
    ...
    mubar = (1 - PH2O[0]/P0)*mubar_dry + (PH2O[0]/P0)*mu_H2O
    P[0] = P0*np.exp(- mubar*grav[0]/(Navo*k*TTT)*(dz/2))
    for i in range(1,nz):
        TTT = pahlevan_poly(P[i-1]/1e6)
        mubar = (1 - PH2O[i-1]/max(P[i-1],1.0e-100))*mubar_dry + (PH2O[i-1]/max(P[i-1],1.0e-100))*mu_H2O
        P[i] = P[i-1]*np.exp(- mubar*grav[i]/(Navo*k*TTT)*(dz))
...
# @nb.njit
def obj(x, nz, z, mubar_dry, M_dry, grav):
    resid, P, T = obj_all(x, nz, z, mubar_dry, M_dry, grav)
    return resid

def make_water_profile(stm, sol, nz = 200, ztop = 500e5, mass = 5.972e27, radius = 6.371e8):
